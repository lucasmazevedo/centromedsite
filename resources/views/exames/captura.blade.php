...
<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
    <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
        ...
        <div class="d-flex align-items-center gap-2 gap-lg-3">
            <button type="button" id="captureImg" onclick="captureImage()" class="btn btn-sm fw-bold btn-primary me-2"> <i class="bi bi-camera"></i> Capturar</button>
            <a href="{{ route('exames.finalizarCaptura', $exame->id) }}" class="btn btn-sm fw-bold btn-success me-2"><i class="bi bi-cloud-check-fill"></i> Finalizar Captura</a>
            <a href="{{ route('exames.cancelarCaptura', $exame->id) }}" class="btn btn-sm fw-bold btn-danger me-2"><i class="bi bi-x-square-fill"></i> Cancelar</a>
        </div>
    </div>
</div>
                                ...
                                <div class="row">
                                    <div class="col-xl-12">
                                        ...
                                        <div class="mt-5 text-center">
                                            ...
                                            <hr>
                                            <div class="row g-3 g-lg-6" id="imagens">
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
@endpush
@push('js_plugins')
<script type="text/javascript" src="{{asset('/assets/js/custom/webcam.js') }}"></script>
@endpush
...
<script>
  ...
  navigator.mediaDevices.enumerateDevices()
    ...
    function captureImage()
    {
        ...
        $.ajax({
            url: "{{ route('exames.capturaImagem', $exame->id) }}",
            type: "POST",
            data: {
                image: picture
            },
    ...
    function getImagesList()
    {
        ...
        $.ajax({
            url: "{{ route('exames.recuperaImagens', $exame->id) }}",
            type: "get",
            beforeSend: function(){
                $('#imagens').html("<h2>Carregando Imagens...</h2>");
            },
            success: function(response) {
                $('#imagens').html(response);
            }
        })
